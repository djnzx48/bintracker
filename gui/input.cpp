...
void Main_Window::scroll_up(const unsigned &amount) {
    if (status.focusBlock) {
        ...
        if (status.get_current_reference_row()
                >= status.get_visible_first_reference_row() + status.visibleReferenceRowsMax) {
            cancel_data_input();
    ...
    if (status.focusBlock) {
        ...
        if (mouseY >= static_cast<int>(status.vScrollbarRefStart)
            ...
            if (static_cast<float>(mouseY) <= status.mouseDragBeginY - status.vScrollbarRefStep
                ...
                status.mouseDragBeginY = static_cast<float>(mouseY);
                scroll_down(lines);
            } else if ((static_cast<float>(mouseY) < status.mouseDragBeginY
                        && status.get_visible_first_reference_row() > 0)
                       || (static_cast<float>(mouseY) > status.mouseDragBeginY
                           && status.vScrollbarRefSize + status.vScrollbarRefStart
                           < referencePanel.totalArea.bottomRight.y - SMALL_BUTTON_SIZE())) {
                ...
                al_draw_filled_rectangle(settings.referencePanelArea.bottomRight.x - SCROLL_BAR_SIZE() - THIN_LINE(),
                                         settings.referencePanelArea.topLeft.y + SMALL_BUTTON_SIZE(),
                                         settings.referencePanelArea.bottomRight.x,
                                         settings.referencePanelArea.bottomRight.y - SMALL_BUTTON_SIZE(),
                                         settings.bgColor);
...
void Main_Window::scroll_drag_h(const int &mouseX) {
    ...
    if (mouseX >= static_cast<int>(status.hScrollbarBlkStart)
        ...
        if (status.get_visible_first_column() > 0) {
            if (status.get_visible_first_column() > 1
                ...
                thresholdLeft = (status.get_current_block_pointer()->columns[status.get_visible_first_column()
                                 - 1].width
                                 * 2.0f / hTotalSize) * paneSize;
            } else {
                thresholdLeft = status.get_current_block_pointer()->columns[status.get_visible_first_column()
                                - 1].width / hTotalSize * paneSize;
            }
        } else {
            thresholdLeft = settings.xres;
        }
        ...
        if (status.get_visible_last_column() + 1 < status.get_current_block_pointer()->columns.size()) {
            if (status.get_visible_last_column() + 2 < status.get_current_block_pointer()->columns.size()
                    ...
                    > status.get_current_block_pointer()->columns[status.get_visible_last_column()].width) {
                thresholdRight = (status.get_current_block_pointer()->columns[status.get_visible_last_column()
                                  + 1].width
                                  * 2.0f / hTotalSize) * paneSize;
            } else {
                thresholdRight = status.get_current_block_pointer()->columns[status.get_visible_last_column()
            ...
            thresholdRight = settings.xres;
        }


        if (status.get_visible_first_column() > 0
                && static_cast<float>(mouseX) <= status.mouseDragBeginX - thresholdLeft) {
            // drag left
    ...
    } else if (mouseX < static_cast<int>(status.hScrollbarBlkStart)) {
        scroll_left();
    } else {
        scroll_right();
    }
    ...
    if (status.get_current_blocktype() == 0) {
        ...
        for (auto pos = static_cast<int>(status.get_current_reference_row()); pos >= 0; pos--) {
            if (currentTune.sequence[static_cast<unsigned>(pos)]
                ...
                status.currentBlocks[0]
                    = status.get_reference_iterator(currentTune.sequence[static_cast<unsigned>(pos)]);
                status.dec_current_reference_row(static_cast<unsigned>(diff));
                print_block_data();
                print_reference_data();
                return;
        ...
        if (nextBlk == 0) {
            nextBlk = currentTune.blockTypes[status.get_current_blocktype()].blocks.size() - 1;
        } else {
            nextBlk -= 1;
        }
...
//  }
    if (status.focusBlock) {
        ...
        if (data == '.') {
            if (status.get_current_cursor_row() != 0) {
                ...
                for (unsigned i = status.get_current_cursor_row(); i > 0; i--) {
                    if (!status.get_current_column_pointer()->columnData[i - 1].dataString.empty()) {
                        currentField->set(status.get_current_column_pointer()
                                            ->columnData[i - 1].dataString, settings.hexMode);

                        if (!status.editLock && status.autoInc > 0) {
                            status.inc_current_cursor_row();
                            print_block_data();
                            if (status.get_current_blocktype() == 0)
                                currentTune.update_last_used(status.get_current_block(),
                                                             status.get_current_cursor_row());
                        } else {
            ...
            // TODO(utz): might need a better check
            ...
            // status.keyLock = 2;      // TODO(utz): temp solution, input is very unstable for some reason
        } else if (!status.editLock && currentField->command->useNoteNames) {
            stringstream noteName;

            if (data == settings.keyC1) {
                noteName << "c" << status.currentOctave;
            } else if (data == 's') {
                noteName << "cis" << status.currentOctave;
            } else if (data == 'x') {
                noteName << "d" << status.currentOctave;
            } else if (data == 'd') {
                noteName << "dis" << status.currentOctave;
            } else if (data == 'c') {
                noteName << "e" << status.currentOctave;
            } else if (data == 'v') {
                noteName << "f" << status.currentOctave;
            } else if (data == 'g') {
                noteName << "fis" << status.currentOctave;
            } else if (data == 'b') {
                noteName << "g" << status.currentOctave;
            } else if (data == 'h') {
                noteName << "gis" << status.currentOctave;
            } else if (data == 'n') {
                noteName << "a" << status.currentOctave;
            } else if (data == 'j') {
                noteName << "ais" << status.currentOctave;
            } else if (data == settings.keyB1) {
                noteName << "b" << status.currentOctave;
            } else if (data == settings.keyC2) {
                noteName << "c" << status.currentOctave + 1;
            } else if (data == '2') {
                noteName << "cis" << status.currentOctave + 1;
            } else if (data == settings.keyD2) {
                noteName << "d" << status.currentOctave + 1;
            } else if (data == '3') {
                noteName << "dis" << status.currentOctave + 1;
            } else if (data == 'e') {
                noteName << "e" << status.currentOctave + 1;
            } else if (data == 'r') {
                noteName << "f" << status.currentOctave + 1;
            } else if (data == '5') {
                noteName << "fis" << status.currentOctave + 1;
            } else if (data == 't') {
                noteName << "g" << status.currentOctave + 1;
            } else if (data == '6') {
                noteName << "gis" << status.currentOctave + 1;
            } else if (data == settings.keyA2) {
                noteName << "a" << status.currentOctave + 1;
            } else if (data == '7') {
                noteName << "ais" << status.currentOctave + 1;
            } else if (data == 'u') {
                noteName << "b" << status.currentOctave + 1;
            } else if (data == 'i') {
                noteName << "c" << status.currentOctave + 2;
            } else if (data == '9') {
                noteName << "cis" << status.currentOctave + 2;
            } else if (data == 'o') {
                noteName << "d" << status.currentOctave + 2;
            } else if (data == '0') {
                noteName << "dis" << status.currentOctave + 2;
            } else if (data == 'p') {
                noteName << "e" << status.currentOctave + 2;
            } else if (data == '1') {
                noteName << "rest";
            } else if (data == 'k') {
                noteName << "noise";
            ...
            if (!noteName.str().empty() && currentTune.musicdataBinary.equates.count(noteName.str())) {
                ...
                if (status.get_current_blocktype() == 0 && soundEmul.playMode < PM_SONG && noteName.str() != "rest")
                    play_row();
            ...
            if (currentField->command->mdCmdForceSubstitution && !currentField->command->isBlkReference) {
                ...
                if (options.empty()) {
                    ...
                    for (auto&& it : currentField->command->substitutionList) {
                        if (userInputString.empty() || it.first.compare(0, userInputString.size(),
                                                                        userInputString) == 0)
                    ...
                    if (status.get_current_blocktype() == 0)
                        currentTune.update_last_used(status.get_current_block(), status.get_current_cursor_row());
                } else {    // TODO(utz): asserting mdCommand::isBlkReference, may this should be enforced?
                    userInputString = auto_complete(options);
                    dropdown.activate(options, userInputString);
                }
        ...
        if (keyEv.type == ALLEGRO_EVENT_KEY_CHAR) {
            if (keyEv.keyboard.keycode >= ALLEGRO_KEY_A && keyEv.keyboard.keycode <= ALLEGRO_KEY_9) {
                if (settings.kbdLang != "EN" || keyEv.keyboard.keycode <= ALLEGRO_KEY_Z) {
                    ...
                    if (keyEv.keyboard.keycode == ALLEGRO_KEY_0) {
                        outputStr += "0";
                    } else if (keyEv.keyboard.keycode == ALLEGRO_KEY_1) {
                        outputStr += "1";
                    } else if (keyEv.keyboard.keycode == ALLEGRO_KEY_2) {
                        outputStr += "2";
                    } else if (keyEv.keyboard.keycode == ALLEGRO_KEY_3) {
                        outputStr += "3";
                    } else if (keyEv.keyboard.keycode == ALLEGRO_KEY_4) {
                        outputStr += "4";
                    } else if (keyEv.keyboard.keycode == ALLEGRO_KEY_5) {
                        outputStr += "5";
                    } else if (keyEv.keyboard.keycode == ALLEGRO_KEY_6) {
                        outputStr += "6";
                    } else if (keyEv.keyboard.keycode == ALLEGRO_KEY_7) {
                        outputStr += "7";
                    } else if (keyEv.keyboard.keycode == ALLEGRO_KEY_8) {
                        outputStr += "8";
                    } else if (keyEv.keyboard.keycode == ALLEGRO_KEY_9) {
                        outputStr += "9";
                    }
                }
                ...
                running = false;
                outputStr = "";
            } else if (keyEv.keyboard.keycode == ALLEGRO_KEY_ENTER) {
                running = false;
            }

            al_flip_display();
        } else if (keyEv.type == ALLEGRO_EVENT_AUDIO_STREAM_FRAGMENT) {
            soundEmul.update_audio();
        }
    }
...
void Main_Window::erase_data_input() {
    ...
    status.editLock = true;
    userInputString = status.get_current_field_pointer()
                      ->dataString.substr(0, status.get_current_field_pointer()->dataString.size() - 1);
    ...
    if (status.editLock) {
        if (status.editBlockName) {
            ...
            if (!userInputString.empty() && validName) {
                ...
                // TODO(utz): blockTypeID is currently ignored in replace_reference();
            ...
            if (field->command->isBlkReference) {
                unsigned targetBt = 0;
                for (; field->command->referenceBlkID != currentTune.config.blockTypes[targetBt].blockConfigID;
                        targetBt++) {}
...
void Main_Window::add_row() {
    if (status.focusBlock) {
        ...
        for (auto&& it : status.get_current_block_pointer()->columns)
            it.columnData.emplace_back(Datablock_Field(it.command));
        print_block_data();
    } else {
        // TODO(utz): must find a better way of detecting wether sequence is available or not
        push_changelog();
...
void Main_Window::delete_row() {
    if (status.focusBlock) {
        ...
        if (status.get_visible_first_reference_row() > 0
                && status.get_visible_first_reference_row() + status.visibleReferenceRowsMax - 1
                >= currentTune.sequence.size())
        ...
        // TODO(utz): find better way of detecting sequence/no sequence
        //  (currently using status.get_current_blocktype())
        // TODO(utz): need a better way of detecting if a block can be deleted or is required
        //  as default reference somewhere (currently using status.get_current_reference_row())
...
void Main_Window::copy_selection() {
    ...
    if (status.focusBlock) {
        ...
        for (unsigned col = status.selectionColumnFirst; col <= status.selectionColumnLast; col++) {
            currentTune.clipboard.emplace_back(
                Datablock_Column(status.get_current_block_pointer()->columns[col].command));
...
void Main_Window::insert_selection() {
    ...
    if (status.focusBlock) {
        ...
        for (auto&& ccol : currentTune.clipboard) {
            if (currentTune.config.isCompatible(*ccol.command,
                                                *status.get_current_block_pointer()->columns[targetCol].command)) {
...
void Main_Window::paste_selection() {
    ...
    if (status.focusBlock) {
        ...
        for (auto&& ccol : currentTune.clipboard) {
            if (currentTune.config.isCompatible(*ccol.command,
                ...
                for (unsigned targetRow = status.get_current_cursor_row();
                    ...
                    status.get_current_block_pointer()->columns[targetCol].columnData[targetRow]
                        = ccol.columnData[sourceRow];
                    sourceRow++;
                }
            }
        ...
        // TODO(utz): move cursor to end of pasted selection
        print_block_data();
    } else {
        if (currentTune.clipboardStatus != SEQUENCE || status.get_current_blocktype() != 0) return;
        push_changelog();
        ...
        for (auto&& it : currentTune.clipboardSequence) {
            if (currentTune.config.seqMaxLength && pos >= currentTune.config.seqMaxLength) break;
            if (pos < currentTune.sequence.size()) {
                currentTune.sequence[pos] = it;
            } else {
                currentTune.sequence.insert(currentTune.sequence.begin() + pos, it);
            }
            pos++;
    ...
    for (auto&& ccol : currentTune.clipboard) {
        if (currentTune.config.isCompatible(*ccol.command,
            ...
            for (unsigned targetRow = status.get_current_cursor_row();
                    targetRow < status.get_current_block_pointer()->columns[0].columnData.size()
                    && sourceRow < ccol.columnData.size(); targetRow++) {
                if (status.get_current_block_pointer()
                        ->columns[targetCol].columnData[targetRow].dataString.empty())
                    status.get_current_block_pointer()->columns[targetCol].columnData[targetRow]
                        = ccol.columnData[sourceRow];
                sourceRow++;
            }
...
// TODO(utz): must handle selections with multiple values as well!
        ...
        if (!(status.get_current_block_pointer()->columns[col].command->mdCmdType == MD_BOOL)
            ...
            if (status.get_current_block_pointer()->columns[col].command->useNoteNames) {
                ...
                while (nextField <= status.selectionRowLast) {
                    float increment;
                    // TODO(utz): evaluate modifiers
                    // TODO(utz): safeguard against fields that aren't note names
                    int64_t startValue = currentTune.musicdataBinary.evalExpr(status.get_current_block_pointer()
                                      ->columns[col].columnData[searchBegin].dataString);
                    int64_t endValue = currentTune.musicdataBinary.evalExpr(status.get_current_block_pointer()
                ...
                while (nextField <= status.selectionRowLast) {
                    ...
                    if (status.get_current_block_pointer()->columns[col].command->allowModifiers) {
                        startValue = currentTune.musicdataBinary.evalExpr(
                                         status.get_current_block_pointer()
                                            ->columns[col].columnData[searchBegin].dataString);
                        endValue = currentTune.musicdataBinary.evalExpr(
                                       status.get_current_block_pointer()
                                        ->columns[col].columnData[nextField].dataString);
                    } else {
                        startValue = strToNum(status.get_current_block_pointer()
                                        ->columns[col].columnData[searchBegin].arg1);
                        endValue = strToNum(status.get_current_block_pointer()
...
void Main_Window::reverse_selection() {
    ...
    for (unsigned col = status.selectionColumnFirst; col <= status.selectionColumnLast; col++) {
        ...
        tempData.insert(tempData.begin(),
                        ...
                        status.get_current_block_pointer()->columns[col].columnData.begin()
                        + status.selectionRowLast + 1);
        ...
        if (!cmd->isBlkReference && !cmd->mdCmdAuto && !cmd->mdCmdForceString) {
            ...
            for (auto&& it : randomData) {
                ...
                if (cmd->mdCmdForceSubstitution) {
                    ...
                    if (it & 1) {
                        status.get_current_block_pointer()
                        ->columns[col].columnData[row].set("true", settings.hexMode);
                    } else {
                        status.get_current_block_pointer()->columns[col].columnData[row].set("false", settings.hexMode);
                    }
...
void Main_Window::fill_data(const unsigned &fillType) {
    ...
    if (status.selectionType == INACTIVE) {
        // TODO(utz): should it do all columns on clipboard?
        if (!currentTune.config.isCompatible(*currentTune.clipboard[0].command,
                                             *status.get_current_column_pointer()->command))
            return;
        ...
        for (unsigned col = status.selectionColumnFirst; col <= status.selectionColumnLast; col++) {
            if (currentTune.config.isCompatible(*currentTune.clipboard[sourceCol].command,
                ...
                for (unsigned row = status.selectionRowFirst; row <= status.selectionRowLast; row++) {
                    if (fillType == REPLACE
                            || status.get_current_block_pointer()->columns[col].columnData[row].dataString.empty())
                        status.get_current_block_pointer()->columns[col].columnData[row]
            ...
            for (unsigned row = status.selectionRowFirst; row <= status.selectionRowLast; row++) {
                if (!status.get_current_block_pointer()->columns[col].columnData[row].arg1.empty()
                    ...
                    if (index < 0 || index >= static_cast<int>(currentTune.freqDividers.size())
                        ...
                        dataString = currentTune.noteNames[index % 12] + numToStr((index / 12), 0, false);
                    }
